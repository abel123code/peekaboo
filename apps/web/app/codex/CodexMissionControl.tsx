...
export function CodexMissionControl({ initialData, setupError, runId }: CodexMissionControlProps) {
  ...
  const latestAssetRun = data?.latestAssetRun || null;
  const isAssetActive = latestAssetRun?.status === "queued" || latestAssetRun?.status === "running";
  const isActive = Boolean(data?.demo.isActive || isAssetActive);
  ...
  const activityLabel = isAssetActive ? "live writer run" : demo?.isActive ? "live Codex run" : latestRun?.status || "ready";
    ...
    <div className="-mx-1 pb-8">
      ...
      {setupError ? (
        ...
        <div className="grid gap-5">
          ...
          <ContentIdeaChooser ideas={contentIdeas} codexRunId={latestRun?.id || ""} assetRuns={data?.assetRuns || []} isRunning={Boolean(demo?.isActive)} isAssetRunning={isAssetActive} />
...
function ContentIdeaChooser({
  ...
  ideas: ContentIdea[];
  codexRunId: string;
  assetRuns: CodexLatestPayload["assetRuns"];
  isRunning: boolean;
  isAssetRunning: boolean;
}) {
  const [selectedId, setSelectedId] = useState("");
  const selected = ideas.find((idea) => idea.id === selectedId) || ideas[0] || null;
  const selectedAssetRun = selected ? assetRuns.find((run) => run.idea_index === selected.index && run.status !== "failed") || null : null;
  const generatedAsset = asRecord(selectedAssetRun?.generated_asset);
  const generatedFiles = asRecord(generatedAsset.files);
  const generatedArticle = typeof generatedFiles.article_md === "string" ? generatedFiles.article_md.trim() : "";
  const hasGeneratedArticle = selectedAssetRun?.status === "completed" && Boolean(generatedArticle);
  const selectedAssetIsRunning = selectedAssetRun?.status === "queued" || selectedAssetRun?.status === "running";
  const shouldShowGenerate = !hasGeneratedArticle && !selectedAssetIsRunning;
  const shouldShowWriterLink = Boolean(selectedAssetRun && (selectedAssetIsRunning || hasGeneratedArticle));
  const writerHref = selectedAssetRun ? `/writer/${selectedAssetRun.id}` : "";
  const canGenerate = Boolean(codexRunId && selected && !isRunning && !isAssetRunning && !hasGeneratedArticle);
    ...
    <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      {selected ? (
        <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
          ...
          {shouldShowGenerate ? (
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-emerald-200 bg-white px-3 py-3">
              <div>
                <div className="text-sm font-semibold text-zinc-950">Ready for final writing</div>
                <p className="mt-0.5 text-xs leading-5 text-zinc-500">
                  Generate the AEO article bundle from this selected idea, source pack, and Reddit case.
                </p>
              </div>
              <form action="/api/aeo-asset" method="post">
                <input type="hidden" name="codexRunId" value={codexRunId} />
                <input type="hidden" name="ideaIndex" value={selected.index} />
                <Button type="submit" disabled={!canGenerate} className="bg-emerald-600 hover:bg-emerald-700">
                  {isAssetRunning ? <Loader2 className="h-4 w-4 animate-spin" /> : <WandSparkles className="h-4 w-4" />}
                  {isAssetRunning ? "Writing..." : "Generate AEO Article"}
                </Button>
              </form>
            </div>
          ) : null}
          {shouldShowWriterLink ? (
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-3">
              <div>
                <div className="text-sm font-semibold text-zinc-950">{selectedAssetIsRunning ? "Writing in progress" : "Generated article ready"}</div>
                <p className="mt-0.5 text-xs leading-5 text-zinc-500">
                  The full writing pipeline and asset tabs are now on the dedicated writer page.
                </p>
              </div>
              <ButtonLink href={writerHref} variant="secondary">
                <FileText className="h-4 w-4" />
                {selectedAssetIsRunning ? "View writer progress" : "View generated article"}
              </ButtonLink>
            </div>
          ) : null}
        </div>
